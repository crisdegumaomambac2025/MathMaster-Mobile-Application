...
public partial class Algebra : ContentPage, IResumableMusicPage
{
    ...
    private const string QuizName = "Algebra";
        ...
        else
        {
            ...
            if (alreadyTaken)
            {
                await DisplayAlert("🎉 You've Already Completed This Quiz!",
                    "Awesome! You've already completed this quiz. Try another one!", "OK");
                await Navigation.PopAsync();
            }
            else
            {
    ...
    private void GenerateQuestions()
    {
        ...
        {
            int type = _random.Next(3); // Three algebra types
            string questionText = "";
            double answer = 0.0;

            switch (type)
            {
                case 0: // Simplifying expressions (e.g., 2x + 3x)
                    int a = _random.Next(1, 6);
                    int b = _random.Next(1, 6);
                    questionText = $"What is the simplified result of {a}x + {b}x when x = 1?";
                    answer = a + b;
                    break;

                case 1: // Solving equations (e.g., 2x + 3 = 7)
                    int coeff = _random.Next(1, 6);
                    int constant = _random.Next(1, 10);
                    int solution = _random.Next(1, 6);
                    int result = coeff * solution + constant;
                    questionText = $"Solve for x: {coeff}x + {constant} = {result}";
                    answer = solution;
                    break;

                case 2: // Advanced word problem with reasoning
                    int notebooks = _random.Next(2, 5);     // e.g., 3 notebooks
                    int pens = _random.Next(2, 5);          // e.g., 4 pens
                    int notebookPrice = _random.Next(20, 40); // ₱30
                    int penPrice = _random.Next(5, 15);     // ₱10
                    int discount = _random.Next(5, 11);     // ₱5 discount if total > ₱100

                    int totalCost = notebooks * notebookPrice + pens * penPrice;
                    bool applyDiscount = totalCost > 100;
                    int finalCost = applyDiscount ? totalCost - discount : totalCost;

                    questionText = $"John bought {notebooks} notebooks at ₱{notebookPrice} each and " +
                                   $"{pens} pens at ₱{penPrice} each. He received a ₱{discount} discount " +
                                   $"if the total exceeded ₱100. How much did he pay?";
                    answer = finalCost;
                    break;
            }
    ...
    private List<double> GenerateOptions(double correctAnswer)
    {
        ...
        {
            double distractor = correctAnswer + _random.Next(-3, 4);
