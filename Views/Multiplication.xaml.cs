...
public partial class Multiplication : ContentPage, IResumableMusicPage
{
    ...
    private const string QuizName = "Multiplication Table";
    ...
    private async void ValidateStudentSession()
    {
        ...
        else
        {
            ...
            if (alreadyTaken)
            {
                ShowInfoDialog("🎉 You've Already Completed This Quiz!", "Great job! You've already taken the Multiplication quiz. Want to try another challenge instead?");
    ...
    {
        // Generate 10 advanced/hard multiplication questions
        var used = new HashSet<string>();
        while (_questions.Count < 10)
        {
            // Harder: 2-digit x 2-digit or 1-digit x 3-digit
            int a, b;
            if (_random.Next(2) == 0)
            {
                a = _random.Next(10, 100); // 2-digit
                b = _random.Next(10, 100); // 2-digit
            }
            else
            {
                a = _random.Next(100, 1000); // 3-digit
                b = _random.Next(2, 10);     // 1-digit
            }
            string question = $"{a} × {b}";
    ...
    private async Task UpdateStudentLevel(string username)
    {
        ...
        int newLevel = currentLevel < 4 ? 4 : currentLevel;
